...
#            kernel_size = int(block['size'])
            if 'size' in block:
                kernel_size_1 = int(block['size'])
                kernel_size_2 = int(block['size'])
                
            elif 'size_h' in block and 'size_w' in block:
                kernel_size_1 = int(block['size_w'])
                kernel_size_2 = int(block['size_h'])
                
            stride = int(block['stride'])
            is_pad = int(block['pad'])
            pad = (kernel_size_1-1)/2 if is_pad else 0
            width = (prev_width + 2*pad - kernel_size_1)/stride + 1
            height = (prev_height + 2*pad - kernel_size_2)/stride + 1
            print('%5d %-6s %4d  %d x %d / %d   %3d x %3d x%4d   ->   %3d x %3d x%4d' % (ind, 'conv', filters, kernel_size_1, kernel_size_2, stride, prev_width, prev_height, prev_filters, width, height, filters))
            prev_width = width
            prev_height = height
...
######################### myself
        elif block['type'] == 'DepthwiseConvolution':
            filters = int(block['filters'])
            if 'size' in block:
                kernel_size_1 = int(block['size'])
                kernel_size_2 = int(block['size'])
            ...
            stride = int(block['stride'])
            is_pad = int(block['pad'])
            pad = (kernel_size_1-1)/2 if is_pad else 0     #pad = (kernel_size-1)/2 if is_pad else 0
            width = (prev_width + 2*pad - kernel_size_1)/stride + 1
            height = (prev_height + 2*pad - kernel_size_2)/stride + 1
            print('%5d %-6s %4d  %d x %d / %d   %3d x %3d x%4d   ->   %3d x %3d x%4d' % (ind, 'conv', filters, kernel_size_1, kernel_size_2, stride, prev_width, prev_height, prev_filters, width, height, filters))
            prev_width = width
...
#########################
            ...
            #print layers
            #print len(out_widths) ,len(out_heights) , len(out_filters)
            if len(layers) == 1:
